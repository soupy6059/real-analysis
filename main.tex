...
\bboxdefn
\begin{defn}[Strongly Equivalent Metrics]
    Let \(d,d'\) be two metrics on a set \(X\).
    We say that \(d\) and \(d'\) are \TB{strongly equivalent} \TU{iff}
...
\begin{defn}[Equivalent Metrics]
    We say \(d,d'\) are \TB{equivalent} \TB{iff}
...
\bboxdefn
\begin{defn}[Dense]
    \((X,d)\). \(A\ssq X\) is \TB{dense} in \(X\) if 
    \[
...
\begin{defn}[Seperable]
    A metric space is \TB{seperable} if there is a countable, dense subset.
\end{defn}
\ebox

\bboxdefn
\begin{defn}[Basis]
    \((X,d)\). \(\beta\ssq\PP(X)\) is a \TB{basis} \TU{iff}
...
\begin{defn}[Second Countable]
    \((X,d)\) is \TB{second countable} \TU{iff} it has a countable basis.
\end{defn}
\ebox
...
\TB{Question:} What should it mean for 
...
\begin{defn}[Homeomorphism]
    \(f:X\to Y\) is a \TB{homeomorphism} \TU{iff}
...
\TB{Question:} what would it mean for \((X,d)\) and 
\((Y,d')\) to be the same.
...
\bboxdefn
\begin{defn}[Isometry]
    \((X,d),\ (Y,d')\) is an \TB{isometry} \TU{iff}
    \[
        d'(f(a),f(b))=d(a,b),\
        \forall a,b\in X
    \]
...
\bboxdefn
\begin{defn}[Isometric Isomorphism]
    If a isometry \(f\) is bijective, it's
    called a \TB{isometric isomorphism}.
\end{defn}
\ebox

\bboxprop
\begin{prop}[Isometries are Continuous and Injective]
    ...
\end{prop}
\ebox
...
\begin{proof}
    ...
    Show 
    \[
        \forall\e>0,\ \exists\de>0\st
        d(x,y)<\de\implies d'(f(x),f(y))<\e
    \]

    We could also show that
    \[
        \exists M>0\st d'(f(x),f(y))\le Md(x,y)
    \]
    which would mean \(f\) is Lipshitz, so it's
    uniformly cts, so it's cts.
    ...
    \]
    \[
        \llet:M=1.\ d'(f(x),f(y))<Md(x,y)\implies\TM{Lip}
    \]
...
\bboxprop
\begin{prop}[Isometric Isomorphisms have Isometric Isomorphic Inverses]
    \(f\) isomet isomor \(\implies f^=\) is isomet isomor
\end{prop}
\ebox
\bboxproof
\begin{proof}
    ...
    Let \(f:X\to Y\) be a isometric isomorphism. So it's bijective.

    Let \(f^=:Y\to X\) be the inverse of \(f\).

    Let \(a,b\in Y\). Show \(d'(a,b)=d(f^=(a),f^=(b))\).
    \[
        d'(a,b)=d'(f(f^=(a)),f(f^=(b)))=d(f^=(a),f^=(b))
    \]
    So \(f^=\) is a isometry, and is bijective.
\end{proof}
\ebox
...
\begin{exam}
    ...
    This is homeomorphic, but not a isometric isomorphism (bij).
\end{exam}
\ebox
